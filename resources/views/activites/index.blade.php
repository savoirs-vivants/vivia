...
@section('content')
    ...
    <div class="flex flex-wrap items-center gap-2 mb-6 pl-2">
        ...
        <a href="{{ route('activites.index', request()->except(['type', 'page'])) }}"
            class="px-3.5 py-1.5 rounded-full text-xs font-bold transition-all {{ !$type ? 'bg-[#222A60] text-white shadow-sm' : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }}">
            Toutes
        </a>
        <a href="{{ route('activites.index', array_merge(request()->except(['type', 'page']), ['type' => 'activite'])) }}"
            class="px-3.5 py-1.5 rounded-full text-xs font-bold transition-all {{ $type === 'activite' ? 'bg-[#222A60] text-white shadow-sm' : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }}">
            Activités
        </a>
        <a href="{{ route('activites.index', array_merge(request()->except(['type', 'page']), ['type' => 'stage'])) }}"
            class="px-3.5 py-1.5 rounded-full text-xs font-bold transition-all {{ $type === 'stage' ? 'bg-amber-500 text-white shadow-sm' : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }}">
            Stages
        </a>
    </div>

    @foreach ($dossiersAvecActivites as $dossier)
        <div class="mb-8 js-folder">
            <button type="button" class="flex items-center gap-3 mb-4 pl-1 w-full text-left js-folder-toggle"
                aria-expanded="false">
                ...
                <div class="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <span class="text-[10px] font-bold bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full flex-shrink-0">
                        {{ $activitesParDossier->get($dossier->id)->count() }}
                    </span>
                </div>
                <svg class="w-4 h-4 text-gray-400 transition-transform flex-shrink-0 js-folder-chevron" fill="none"
            ...
            <div
                class="js-folder-content hidden grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-5">
                @foreach ($activitesParDossier->get($dossier->id) as $activite)
    ...
    @if ($activitesSansDossier->isNotEmpty() || $activites->isEmpty())
        ...
        @if ($dossiersAvecActivites->isNotEmpty())
            <div class="flex items-center gap-3 mb-4 pl-1">
                ...
                <span
                    class="text-[10px] font-bold bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full">{{ $activitesSansDossier->count() }}</span>
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-5">
           ...
           text-gray-400 hover:border-[#16987C] hover:text-[#16987C] transition-all group min-h-[200px]">
                    <div
                        class="w-11 h-11 rounded-full border-2 border-dashed border-current flex items-center justify-center group-hover:scale-110 transition-transform">
            ...
            @if ($archives->count() > 0)
                <button type="button" data-panel="archives"
           ...
           text-gray-400 hover:border-gray-300 hover:text-gray-600 transition-all group min-h-[200px]">
                    <div class="relative">
                        <div
                            class="w-14 h-14 rounded-2xl bg-gray-200 flex items-center justify-center group-hover:scale-110 transition-transform">
                        ...
                        <div
                            class="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-rose-500 text-white text-[10px] font-black flex items-center justify-center border-2 border-white shadow-sm">
                            {{ $archives->count() }}
                        </div>
                    </div>
    ...
    <div id="archives-modal" class="fixed inset-0 z-50 hidden">
        <div class="absolute inset-0 bg-gray-900/40 backdrop-blur-sm transition-opacity" data-panel="archives"></div>

        <div id="archives-panel"
            class="absolute inset-y-0 right-0 max-w-md w-full bg-white shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-4">
                @foreach ($archives as $archive)
                    <div
                        class="p-4 rounded-xl border border-gray-100 bg-white hover:border-gray-200 transition-colors flex items-center justify-between group shadow-sm">
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                <span
                                    class="px-2 py-0.5 rounded text-[10px] font-bold uppercase {{ $archive->est_stage ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600' }}">
                        ...
                        <form action="{{ route('activites.toggleArchive', $archive) }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="flex items-center gap-2 px-3 py-1.5 bg-gray-50 hover:bg-[#16987C] hover:text-white text-gray-500 text-xs font-bold rounded-lg transition-colors border border-gray-100 hover:border-[#16987C]">
    ...
    <div id="dossiers-modal" class="fixed inset-0 z-50 hidden">
        <div class="absolute inset-0 bg-gray-900/40 backdrop-blur-sm" data-panel="dossiers"></div>

        <div id="dossiers-panel"
            class="absolute inset-y-0 right-0 max-w-sm w-full bg-white shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                <div class="flex items-center gap-3">
                ...
                <button type="button" data-panel="dossiers"
                    class="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                ...
                @else
                    <div class="space-y-2">
                        @foreach ($dossiers as $dossier)
                            <div
                                class="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-white hover:border-gray-200 transition-colors group">
                                <div class="flex items-center gap-3">
                                    <div class="w-7 h-7 rounded-lg bg-[#222A60]/8 flex items-center justify-center">
                                        <svg class="w-3.5 h-3.5 text-[#222A60]" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z" />
                                        </svg>
                                ...
                                <form action="{{ route('dossiers.destroy', $dossier) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="p-2 text-gray-300 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
