                        ...
                        <div class="p-5 md:p-6" x-data="{ modalCotisation: {{ $paiement1Done ? 'true' : 'false' }} }">
                            ...
                            <div x-show="modalCotisation" x-transition
                                ...
                                <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6" @click.outside="false">
                                    @php
                                        $isEsrPme = ($formData['statut_juridique'] ?? '') === 'esr_pme';
                                        $nomOngletStructure = $isEsrPme ? 'Cotisation ESR / PME' : 'Cotisation TPE / Association';
                                    @endphp

                                    <div class="text-center mb-5">
                                        <div
                                            class="w-14 h-14 rounded-2xl bg-teal-50 border border-teal-100 flex items-center justify-center text-3xl mx-auto mb-3">
                                            🏛️</div>
                                        <h3 class="text-lg font-bold text-gray-900">
                                            @if (($formData['type_activite'] ?? '') === 'ressourcerie')
                                                Ressourcerie réglée !
                                            @else
                                                Dernière étape — Cotisation
                                            @endif
                                        </h3>
                                        <p class="text-gray-500 text-sm mt-1">
                                            Réglez la cotisation annuelle <strong>({{ $montantStructure }} €)</strong>
                                            via le formulaire HelloAsso.
                                        </p>
                                    </div>
                                    ...
                                    <div
                                        ...
                                        <div>
                                            <p class="text-sm font-semibold text-amber-900">Choisissez le bon onglet</p>
                                            <p class="text-sm text-amber-700 mt-0.5 leading-relaxed">
                                                Sur la page HelloAsso, sélectionnez l'onglet
                                                <strong>« {{ $nomOngletStructure }} »</strong>,
                                                puis revenez ici et cliquez sur le bouton ci-dessous.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    @error('helloasso2')
                                        <div
                                            class="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700 font-medium">
                                            ❌ {{ $message }}
                                        </div>
                                    @enderror
                                    ...
                                    <div x-data="cotisationPaiement()" x-init="init()">
                                        ...
                                        <div x-show="dejaClique" x-transition class="mt-4 space-y-3">
                                            <div
                                                class="p-4 bg-teal-50 border border-teal-200 rounded-xl text-sm text-teal-800">
                                                <p class="font-semibold mb-1">La page HelloAsso s'est ouverte dans un
                                                    nouvel onglet.</p>
                                                <p class="text-xs text-teal-700">Une fois le paiement finalisé, revenez
                                                    ici et cliquez sur le bouton ci-dessous.</p>
                                            </div>
                                            <form action="{{ route('adhesion.verifier.cotisation', $token) }}"
                                                method="POST">
                                                @csrf
                                                <button type="submit"
                                    ...
                                    <p class="text-center text-xs text-gray-400 mt-3">Paiement sécurisé via HelloAsso
                                    </p>
                                </div>
                            </div>
                            ...
                            @if ($totalRessourcerieStructure !== null)
                                ...
                                <div class="p-5 bg-teal-50 border border-teal-200 rounded-2xl mb-6">
                                    <p class="text-xs font-bold text-teal-700 uppercase tracking-wide mb-3">Items
                                        sélectionnés</p>
                                    @foreach ($ressourcerieSelectionnees as $item)
                                        <div
                                            class="flex items-center justify-between py-1.5 border-b border-teal-100 last:border-0">
                                            <span class="text-sm text-teal-900">{{ $item->nom }}</span>
                                            <span class="text-sm font-bold text-teal-700">{{ $item->prix }} €</span>
                                        </div>
                                    @endforeach
                                    <p class="text-xs text-teal-600 mt-2">La cotisation annuelle
                                        ({{ $montantStructure }} €) sera réglée dans un second temps.</p>
                                </div>
                            @else
                    ...
                    @else
                        <div class="p-5 md:p-6" x-data="{ modalAdhesion: {{ $paiement1Done ? 'true' : 'false' }} }">
                                    ...
                                    <div x-data="cotisationPaiement()" x-init="init()">
                                        ...
                                        <div x-show="dejaClique" x-transition class="mt-4 space-y-3">
                                            <div
                                                ...
                                                <p class="font-semibold mb-1">La page HelloAsso s'est ouverte dans un
                                                    nouvel onglet.</p>
                                                <p class="text-xs text-teal-700">Une fois le paiement finalisé sur
                                                    HelloAsso, revenez ici et cliquez sur le bouton ci-dessous.</p>
                                            </div>
                            ...
                            <div class="mb-5">
                                ...
                                <p class="text-gray-400 mt-1 text-sm">Comment souhaitez-vous régler votre adhésion ?
                                </p>
                            </div>
                                ...
                                <div x-data="{ modePaiement: '{{ $formData['mode_paiement'] ?? 'helloasso' }}' }">
                                    ...
                                    <div x-show="modePaiement === 'interne'" x-transition style="display: none;"
                                        ...
                                        <p class="text-sm font-medium text-slate-700 mb-4 leading-relaxed">
                                            Pour finaliser votre adhésion, merci de contacter notre équipe afin de
                                            convenir d'un
                                            rendez-vous :
                                        </p>
                                        ...
                                        <p class="text-xs text-gray-500 mt-4 font-semibold">Modes acceptés : chèque,
                                            espèces,
                                    ...
                                    <div x-show="modePaiement === 'helloasso'" x-transition style="display: none;"
                                        ...
                                        <p
                                            class="text-sm font-bold text-slate-900 flex items-center gap-3 leading-relaxed">
                                            <span class="text-xl">🔒</span>
                                            Vous allez être redirigé·e vers la plateforme sécurisée HelloAsso pour
                                            procéder au
                                            paiement.
                                        </p>
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between pt-4 border-t border-gray-100 mt-1">
                                        ...
                                        <button type="submit" class="{{ $btn }}">
                                            <span x-show="modePaiement === 'helloasso'">Continuer vers HelloAsso
                                                🔒</span>
                                            <span x-show="modePaiement === 'interne'">Valider mon inscription ✓</span>
                                            <span x-show="modePaiement === 'pass_culture'">Valider mon inscription
                                                ✓</span>
                                        </button>
                                    </div>
                                </div>
