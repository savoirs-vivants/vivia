...
@php
    $field = 'w-full rounded-xl border border-gray-300 px-4 py-3.5 text-gray-800 bg-white focus:outline-none focus:ring-4 focus:ring-teal-600/20 focus:border-teal-600 transition-all shadow-sm text-base';
    $label = 'block text-sm font-bold text-slate-700 mb-2';
    $card  = 'border-2 rounded-2xl p-5 transition-all duration-200 h-full flex flex-col bg-white hover:shadow-md cursor-pointer';
    $btn   = 'inline-flex items-center justify-center gap-2 bg-slate-900 text-white font-bold px-8 py-3.5 rounded-xl hover:bg-teal-600 focus:ring-4 focus:ring-teal-600/30 transition-all duration-200 shadow-md hover:shadow-lg text-sm';
    $btnBack = 'inline-flex items-center justify-center gap-2 text-slate-600 font-semibold px-5 py-3.5 rounded-xl border border-gray-200 hover:bg-gray-100 hover:text-slate-900 transition-all duration-200 text-sm';
...
<div class="min-h-screen bg-slate-50 py-8 px-4" style="font-family: 'Space Grotesk', sans-serif;">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="text-center mb-8">
            <div class="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-4 bg-slate-900 shadow-lg text-white">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6">
            <div class="flex items-center justify-between mb-3">
                <span class="text-sm text-gray-500">Étape <span class="font-bold text-slate-900">{{ $currentNum }}</span> sur <span class="font-bold">{{ $totalSteps }}</span></span>
                ...
                @foreach($path as $i => $s)
                    ...
                    <div class="flex items-center {{ !$isLast ? 'flex-1 min-w-0' : '' }}">
                        <div class="flex flex-col items-center shrink-0">
                            ...
                            <span class="text-[10px] mt-1.5 text-center uppercase tracking-wide max-w-[60px]
                                @if($isCurrent) text-slate-900 font-bold
                                @elseif($isDone) text-teal-600 font-bold
                                @else text-gray-400 font-semibold
                                @endif">
                                {{ $stepMeta[$s]['label'] }}
                            </span>
                        </div>
                        @if(!$isLast)
                        <div class="flex-1 h-0.5 mx-2 transition-all {{ $isDone ? 'bg-teal-600' : 'bg-gray-200' }}"></div>
                        @endif
                    </div>
                @endforeach
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div x-data="{ statut: '{{ $formData['is_adherent'] ?? '' }}' }">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                            <label class="cursor-pointer block group">
                                <input type="radio" name="is_adherent" value="oui" x-model="statut" class="sr-only">
                                <div :class="statut === 'oui' ? 'border-teal-600 bg-teal-50 ring-2 ring-teal-600/20' : 'border-gray-200 group-hover:border-slate-900'"
                                    ...
                                    <div class="mt-4 w-6 h-6 rounded-full mx-auto border-2 flex items-center justify-center transition-colors"
                                         :class="statut === 'oui' ? 'border-teal-600 bg-teal-600' : 'border-gray-300'">
                                        <svg x-show="statut === 'oui'" class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                                    </div>
                                </div>
                            </label>

                            <label class="cursor-pointer block group">
                                <input type="radio" name="is_adherent" value="non" x-model="statut" class="sr-only">
                                <div :class="statut === 'non' ? 'border-teal-600 bg-teal-50 ring-2 ring-teal-600/20' : 'border-gray-200 group-hover:border-slate-900'"
                                     class="{{ $card }} items-center text-center">
                                    <div class="text-5xl mb-3">🆕</div>
                                    ...
                                    <div class="mt-4 w-6 h-6 rounded-full mx-auto border-2 flex items-center justify-center transition-colors"
                                         :class="statut === 'non' ? 'border-teal-600 bg-teal-600' : 'border-gray-300'">
                                        <svg x-show="statut === 'non'" class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                                    </div>
                                </div>
                            </label>
                        </div>
                        ...
                        <div x-show="statut === 'oui'" x-transition:enter="transition ease-out duration-200"
                            ...
                            <input type="text" name="numero_adherent" value="{{ $formData['numero_adherent'] ?? '' }}"
                                   placeholder="Ex : ADH-2025-0042" class="{{ $field }}">
                            <p class="text-xs text-gray-500 mt-2 font-medium">Vous trouverez ce numéro sur le mail confirmant votre adhésion.</p>
                        </div>
            ...
            <div class="p-6 md:p-8">
                ...
                <form action="{{ route('adhesion.next', $token) }}" method="POST">
                    ...
                    <div x-data="{ activite: '{{ $formData['type_activite'] ?? '' }}' }">
                        @php
                        $options = [
                            ['value' => 'atelier',     'label' => 'Inscription à un atelier',       'icon' => '🔧', 'desc' => 'Ateliers créatifs, techniques ou artistiques'],
                            ['value' => 'pass_culture','label' => 'Pass Culture',                   'icon' => '🎫', 'desc' => 'Utiliser votre Pass Culture'],
                            ['value' => 'stage',       'label' => 'Inscription à un stage',         'icon' => '🎭', 'desc' => 'Stages intensifs sur plusieurs jours'],
                            ['value' => 'club_maker',  'label' => 'Club Maker',                     'icon' => '🔨', 'desc' => 'Rejoindre le club des makers'],
                            ['value' => 'soutien',     'label' => 'Inscription par soutien',        'icon' => '💛', 'desc' => 'Soutenir financièrement l\'association'],
                            ['value' => 'recherche',   'label' => 'Recherche participative',        'icon' => '🔬', 'desc' => 'Participer à un programme de recherche'],
                        ];
                        @endphp
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                            ...
                            <label class="cursor-pointer block group">
                                ...
                                <div :class="activite === '{{ $opt['value'] }}' ? 'border-teal-600 bg-teal-50 ring-2 ring-teal-600/20' : 'border-gray-200 group-hover:border-slate-900'"
                                     class="{{ $card }}">
                                    ...
                                    <div class="mt-3 w-5 h-5 rounded-full border-2 flex items-center justify-center self-end transition-colors"
                                         :class="activite === '{{ $opt['value'] }}' ? 'border-teal-600 bg-teal-600' : 'border-gray-300'">
                                        <svg x-show="activite === '{{ $opt['value'] }}'" class="w-2.5 h-2.5 text-white" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                                    </div>
                                </div>
                            </label>
                            @endforeach
                ...
                <form action="{{ route('adhesion.next', $token) }}" method="POST">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            ...
                            <label class="cursor-pointer block group">
                                <input type="radio" name="genre" value="{{ $val }}"
                                       {{ ($formData['genre'] ?? '') === $val ? 'checked' : '' }} class="sr-only peer">
                                <div class="border-2 rounded-xl p-3 text-center peer-checked:border-teal-600 peer-checked:bg-teal-50 peer-checked:ring-2 ring-teal-600/20 border-gray-200 group-hover:border-slate-900 transition-all">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                        <div>
                            ...
                            <input type="text" name="nom" value="{{ $formData['nom'] ?? '' }}" placeholder="Votre nom" required class="{{ $field }}">
                        </div>
                        <div>
                            <label class="{{ $label }}">Prénom *</label>
                            <input type="text" name="prenom" value="{{ $formData['prenom'] ?? '' }}" placeholder="Votre prénom" required class="{{ $field }}">
                        </div>
                    </div>

                    <div class="mb-4">
                        <label class="{{ $label }}">Adresse *</label>
                        <input type="text" name="adresse" value="{{ $formData['adresse'] ?? '' }}" placeholder="Numéro et nom de rue" required class="{{ $field }}">
                    </div>
                    <div class="grid grid-cols-2 gap-4 mb-5">
                        <div>
                            <label class="{{ $label }}">Code postal *</label>
                            <input type="text" name="code_postal" value="{{ $formData['code_postal'] ?? '' }}" placeholder="67000" required class="{{ $field }}">
                        </div>
                        <div>
                            <label class="{{ $label }}">Ville *</label>
                            <input type="text" name="ville" value="{{ $formData['ville'] ?? '' }}" placeholder="Strasbourg" required class="{{ $field }}">
                        </div>
                    </div>

                    <div class="mb-5">
                        <label class="{{ $label }}">🎂 Date de naissance *</label>
                        <input type="date" name="date_naiss" value="{{ $formData['date_naiss'] ?? '' }}" required class="{{ $field }}">
                        <p class="text-xs text-gray-500 mt-2 font-medium">Cette information détermine automatiquement les étapes suivantes.</p>
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                        <div>
                            <label class="{{ $label }}">📞 Téléphone</label>
                            <input type="tel" name="tel" value="{{ $formData['tel'] ?? '' }}" placeholder="06 00 00 00 00" class="{{ $field }}">
                        </div>
                        <div>
                            <label class="{{ $label }}">📧 Email</label>
                            <input type="email" name="mail" value="{{ $formData['mail'] ?? '' }}" placeholder="exemple@mail.com" class="{{ $field }}">
                        </div>
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <select name="regime_social" class="{{ $field }}">
                            ...
                            <option value="{{ $r }}" {{ ($formData['regime_social'] ?? '') === $r ? 'selected' : '' }}>{{ $r }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="p-5 bg-slate-50 rounded-2xl border border-slate-200 mb-6 space-y-4">
                        ...
                        <label class="flex items-start gap-3 cursor-pointer group">
                            <input type="checkbox" name="bulletin" value="1" {{ ! empty($formData['bulletin']) ? 'checked' : '' }} class="{{ $check }} mt-0.5">
                            <span class="text-sm text-gray-700 leading-relaxed">
                                <strong class="text-slate-900">Bulletin d'information</strong> — J'accepte de recevoir les bulletins et actualités de l'association par email.
                            </span>
                        </label>
                        <label class="flex items-start gap-3 cursor-pointer group">
                            <input type="checkbox" name="communication" value="1" {{ ! empty($formData['communication']) ? 'checked' : '' }} class="{{ $check }} mt-0.5">
                            <span class="text-sm text-gray-700 leading-relaxed">
                                <strong class="text-slate-900">Droit à l'image</strong> — J'autorise l'association à photographier l'adhérent·e et à diffuser ces images.
                            </span>
                        </label>
                    </div>
            ...
            <div class="p-6 md:p-8">
                ...
                <form action="{{ route('adhesion.next', $token) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <div class="mb-4 p-4 bg-teal-50 border border-teal-200 rounded-xl flex items-center gap-3">
                            <span class="text-teal-600 text-xl">✅</span>
                            <span class="text-sm font-semibold text-teal-800">Fichier déjà envoyé. Vous pouvez le remplacer ci-dessous.</span>
                        </div>
                        @endif
                        <div class="border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center hover:border-teal-600 hover:bg-teal-50/50 transition-all cursor-pointer group"
                            ...
                            <input type="file" name="carnet_sante" accept="image/*,.pdf"
                                   class="hidden" x-ref="fileInput"
                                   @change="preview = $event.target.files[0] ? URL.createObjectURL($event.target.files[0]) : null">
                            <template x-if="!preview">
                                <div>
                                    <div class="text-5xl mb-3 group-hover:scale-110 transition-transform">📁</div>
                                    <p class="text-base font-bold text-slate-700">Cliquez pour déposer le carnet</p>
                                    <p class="text-sm text-gray-400 mt-1 font-medium">JPG, PNG ou PDF — max 10 Mo</p>
                                </div>
                            </template>
                            <template x-if="preview">
                                <img :src="preview" class="max-h-48 mx-auto rounded-xl shadow-sm object-contain border border-gray-200">
                            </template>
                        </div>
                    </div>
                ...
                <form action="{{ route('adhesion.next', $token) }}" method="POST">
                    ...
                    <div x-data="{ occupation: '{{ $formData['occupation'] ?? '' }}' }">
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-5">
                            @foreach([
                                ['val' => 'Maternelle', 'icon' => '🧸', 'desc' => '3 à 5 ans'],
                                ['val' => 'Primaire',   'icon' => '📖', 'desc' => '6 à 10 ans'],
                                ['val' => 'Collège',    'icon' => '🎒', 'desc' => '11 à 14 ans'],
                                ['val' => 'Lycée',      'icon' => '🎓', 'desc' => '15 à 17 ans'],
                            ] as $o)
                            <label class="cursor-pointer block group">
                                <input type="radio" name="occupation" value="{{ $o['val'] }}" x-model="occupation" class="sr-only">
                                <div :class="occupation === '{{ $o['val'] }}' ? 'border-teal-600 bg-teal-50 ring-2 ring-teal-600/20' : 'border-gray-200 group-hover:border-slate-900'"
                                     class="border-2 rounded-2xl p-4 text-center transition-all">
                                    <div class="text-3xl mb-1">{{ $o['icon'] }}</div>
                                    <div class="font-bold text-sm text-slate-900">{{ $o['val'] }}</div>
                                    <div class="text-xs text-gray-500 font-medium mt-1">{{ $o['desc'] }}</div>
                                </div>
                            </label>
                            @endforeach
                        </div>
                        ...
                        <div x-show="occupation !== ''" x-transition class="mb-5">
                            ...
                            <input type="text" name="etablissement" value="{{ $formData['etablissement'] ?? '' }}" placeholder="Nom de l'école / collège / lycée" class="{{ $field }}">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
                        ...
                        <label class="cursor-pointer block group">
                            <input type="radio" name="occupation" value="{{ $o['val'] }}" {{ ($formData['occupation'] ?? '') === $o['val'] ? 'checked' : '' }} class="sr-only peer">
                            <div class="border-2 rounded-xl p-3 text-center transition-all peer-checked:border-teal-600 peer-checked:bg-teal-50 peer-checked:ring-2 ring-teal-600/20 border-gray-200 group-hover:border-slate-900">
            ...
            <div class="p-6 md:p-8">
                ...
                <div class="mb-6">
                    <h2 class="text-2xl font-bold text-slate-900">
                        @if($typeActivite === 'atelier') Choisissez votre atelier 🔧
                        @else Choisissez votre stage 🎭 @endif
                    </h2>
                    <p class="text-gray-500 mt-1">
                        @if($typeActivite === 'atelier') Sélectionnez le ou les ateliers auxquels vous souhaitez vous inscrire
                        @else Sélectionnez le stage qui vous intéresse @endif
                    </p>
                </div>
                ...
                <form action="{{ route('adhesion.next', $token) }}" method="POST">
                    ...
                    @php
                        ...
                        if (!is_array($selectedActivites)) $selectedActivites = [];
                    @endphp
            ...
            <div class="p-6 md:p-8">
                <div class="mb-6">
                    ...
                    <p class="text-gray-500 mt-1">Souhaitez-vous vous investir dans l'association ? (facultatif)</p>
                </div>
                ...
                <form action="{{ route('adhesion.next', $token) }}" method="POST">
                    ...
                    <div class="mb-8">
                        <h3 class="font-bold text-slate-900 mb-4 flex items-center gap-3">
                            <span class="w-8 h-8 rounded-xl bg-slate-900 text-white text-sm flex items-center justify-center font-bold shadow-md">1</span>
                            Actions bénévoles
                        </h3>
                        @php
                        $benevoleOptions = [
                        ...
                        if (!is_array($actionsSelectionnees)) $actionsSelectionnees = [];
                        @endphp
                        <div class="space-y-3">
                            @foreach($benevoleOptions as $option => $icon)
                            <label class="flex items-center gap-4 p-4 border-2 border-gray-100 rounded-xl cursor-pointer hover:border-teal-600 hover:bg-teal-50 transition-all group">
                                <input type="checkbox" name="actions_benevoles[]" value="{{ $option }}"
                    ...
                    <div class="p-6 bg-slate-50 rounded-2xl border border-slate-200 mb-6">
                        <h3 class="font-bold text-slate-900 mb-3 flex items-center gap-3">
                            <span class="w-8 h-8 rounded-xl bg-slate-900 text-white text-sm flex items-center justify-center font-bold shadow-md">2</span>
                        ...
                        <div class="flex flex-col sm:flex-row gap-4">
                            <label class="flex items-center gap-3 p-3 bg-white border border-gray-200 rounded-xl cursor-pointer hover:border-teal-600 transition-all flex-1">
                                <input type="radio" name="participation_manif" value="1" {{ ($formData['participation_manif'] ?? '') === '1' ? 'checked' : '' }} class="{{ $radio }}">
                                <span class="text-sm font-bold text-slate-900">✅ Oui, intéressé·e</span>
                            </label>
                            <label class="flex items-center gap-3 p-3 bg-white border border-gray-200 rounded-xl cursor-pointer hover:border-slate-900 transition-all flex-1">
                                <input type="radio" name="participation_manif" value="0" {{ ($formData['participation_manif'] ?? '') === '0' ? 'checked' : '' }} class="{{ $radio }}">
            ...
            <div class="p-6 md:p-8">
                <div class="mb-6">
                    ...
                    <p class="text-gray-500 mt-1">Responsables légaux de <strong class="text-slate-900">{{ ($formData['prenom'] ?? '') . ' ' . ($formData['nom'] ?? '') }}</strong></p>
                </div>
                ...
                <form action="{{ route('adhesion.next', $token) }}" method="POST" id="form-tuteurs">
                    ...
                    <div x-data="tuteurManager()" x-init="init()">
                        <template x-for="(tuteur, i) in tuteurs" :key="i">
                            <div class="border-2 border-gray-200 rounded-3xl p-6 mb-5 bg-white relative hover:border-slate-300 transition-colors">
                                <div class="flex items-center justify-between mb-5">
                                    <h3 class="font-bold text-slate-900 text-lg flex items-center gap-3">
                                        <span class="w-10 h-10 rounded-xl bg-slate-900 text-white flex items-center justify-center font-bold shadow-md" x-text="'T' + (i+1)"></span>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        <label class="{{ $label }}">Nom *</label>
                                        <input type="text" :name="'tuteurs[' + i + '][nom]'" x-model="tuteur.nom" placeholder="Nom de famille" class="{{ $field }}" required>
                                    </div>
                                    <div>
                                        <label class="{{ $label }}">Prénom *</label>
                                        <input type="text" :name="'tuteurs[' + i + '][prenom]'" x-model="tuteur.prenom" placeholder="Prénom" class="{{ $field }}" required>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                                    <div>
                                        <label class="{{ $label }}">📞 Téléphone</label>
                                        <input type="tel" :name="'tuteurs[' + i + '][tel]'" x-model="tuteur.tel" placeholder="06 00 00 00 00" class="{{ $field }}">
                                    </div>
                                    <div>
                                        <label class="{{ $label }}">📧 Email</label>
                                        <input type="email" :name="'tuteurs[' + i + '][mail]'" x-model="tuteur.mail" placeholder="email@exemple.com" class="{{ $field }}">
                                    </div>
                                </div>

                                <div class="mb-5 p-4 bg-blue-50 rounded-2xl border border-blue-100">
                                    <label class="{{ $label }} text-blue-800">✏️ Nom de l'enfant (validation)</label>
                                    <input type="text" :name="'tuteurs[' + i + '][nom_enfant]'" x-model="tuteur.nom_enfant" class="{{ $field }} bg-white">
                                </div>

                                <div class="space-y-3 mb-6 p-5 bg-slate-50 border border-slate-200 rounded-2xl">
                                    <label class="flex items-center gap-3 cursor-pointer group">
                                        <input type="checkbox" :name="'tuteurs[' + i + '][adhere]'" value="1" :checked="tuteur.adhere" @change="tuteur.adhere = $event.target.checked" class="{{ $check }}">
                                        <span class="text-sm font-semibold text-slate-800">J'adhère à l'association SV</span>
                                    </label>
                                    <label class="flex items-center gap-3 cursor-pointer group">
                                        <input type="checkbox" :name="'tuteurs[' + i + '][rentre_fin]'" value="1" :checked="tuteur.rentre_fin" @change="tuteur.rentre_fin = $event.target.checked" class="{{ $check }}">
                                        <span class="text-sm font-semibold text-slate-800">J'autorise l'enfant à rentrer seul·e à la fin de l'activité</span>
                                    </label>
                                    <label class="flex items-center gap-3 cursor-pointer group">
                                        <input type="checkbox" :name="'tuteurs[' + i + '][rentre_annul]'" value="1" :checked="tuteur.rentre_annul" @change="tuteur.rentre_annul = $event.target.checked" class="{{ $check }}">
                                        <span class="text-sm font-semibold text-slate-800">J'autorise l'enfant à rentrer seul·e en cas d'annulation</span>
                                    </label>
                                </div>

                                <div class="mb-5">
                                    <label class="{{ $label }}">📅 Date</label>
                                    <input type="date" :name="'tuteurs[' + i + '][date_signature]'" x-model="tuteur.date_signature" class="{{ $field }} max-w-xs">
                                </div>

                                <div>
                                    <label class="{{ $label }}">✍️ Signature du/de la tuteur·trice</label>
                                    <div class="border-2 border-dashed border-gray-300 rounded-2xl p-2 bg-gray-50 relative overflow-hidden" style="max-width: 400px;">
                                        <canvas :id="'canvas-tuteur-' + i" class="w-full touch-none bg-white rounded-xl cursor-crosshair shadow-sm border border-gray-100" style="height: 120px; display: block;"></canvas>
                                        <button type="button" @click="clearCanvas(i)" class="absolute top-4 right-4 bg-white border border-gray-200 text-xs font-bold text-gray-500 hover:text-red-500 hover:border-red-200 px-2 py-1 rounded shadow-sm transition">
                                            Effacer
                                        </button>
                                    </div>
                                    <input type="hidden" :name="'tuteurs[' + i + '][signature]'" :id="'sig-data-tuteur-' + i" x-model="tuteur.signature">
                                </div>
                            </div>
                        </template>
                ...
                <form action="{{ route('adhesion.next', $token) }}" method="POST" id="form-signature">
                    ...
                    <div class="p-5 bg-amber-50 rounded-2xl border border-amber-200 mb-6 flex items-start gap-4">
                        ...
                        <p class="text-sm font-bold text-amber-800 leading-relaxed">
                            En signant ce formulaire, vous certifiez l'exactitude des informations renseignées et acceptez le règlement intérieur de l'association.
                        </p>
                    </div>

                    <div class="mb-6">
                        <label class="{{ $label }}">📅 Date *</label>
                        <input type="date" name="date_signature_adherent" value="{{ $formData['date_signature_adherent'] ?? date('Y-m-d') }}" required class="{{ $field }} max-w-xs">
                    </div>

                    <div class="mb-8">
                        <label class="{{ $label }}">✍️ Signature *</label>
                        <div class="border-2 border-dashed border-gray-300 rounded-2xl bg-gray-50 p-2 relative" style="max-width: 500px;">
                            <canvas id="canvas-adherent" class="w-full touch-none bg-white rounded-xl cursor-crosshair block border border-gray-100 shadow-sm" style="height: 180px;"></canvas>
                            <button type="button" id="clear-sig-adherent" class="absolute top-4 right-4 bg-white border border-gray-200 text-xs font-bold text-gray-500 hover:text-red-500 hover:border-red-200 px-3 py-1.5 rounded-lg shadow-sm transition">
                                Effacer
                            </button>
                        </div>
                        <input type="hidden" name="signature_adherent" id="sig-data-adherent" value="{{ $formData['signature_adherent'] ?? '' }}">
                    </div>
            ...
            <div class="p-6 md:p-8">
                ...
                <form action="{{ route('adhesion.next', $token) }}" method="POST">
                    ...
                    <div x-data="{ modePaiement: '{{ $formData['mode_paiement'] ?? '' }}' }">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                            <label class="cursor-pointer block group">
                                <input type="radio" name="mode_paiement" value="helloasso" x-model="modePaiement" class="sr-only">
                                <div :class="modePaiement === 'helloasso' ? 'border-teal-600 bg-teal-50 ring-2 ring-teal-600/20' : 'border-gray-200 group-hover:border-slate-900'"
                                     class="{{ $card }} items-center text-center">
                                    ...
                                    <span class="inline-block mt-4 text-xs font-bold bg-slate-100 text-slate-500 px-3 py-1.5 rounded-full uppercase tracking-wider">Bientôt disponible</span>
                                </div>
                            </label>

                            <label class="cursor-pointer block group">
                                <input type="radio" name="mode_paiement" value="interne" x-model="modePaiement" class="sr-only">
                                <div :class="modePaiement === 'interne' ? 'border-teal-600 bg-teal-50 ring-2 ring-teal-600/20' : 'border-gray-200 group-hover:border-slate-900'"
                                     class="{{ $card }} items-center text-center">
                                    <div class="text-4xl mb-3">🤝</div>
                        ...
                        <div x-show="modePaiement === 'interne'" x-transition class="p-6 bg-slate-50 border border-slate-200 rounded-2xl mb-6">
                            ...
                            <a href="mailto:contact@savoirsvivants.fr" class="inline-flex items-center gap-2 bg-slate-900 text-white text-sm font-bold px-5 py-3 rounded-xl hover:bg-teal-600 shadow-md transition-colors">
                        ...
                        <div x-show="modePaiement === 'helloasso'" x-transition class="p-5 bg-gray-50 border border-gray-200 rounded-2xl mb-6">
            ...
            <div class="p-8 md:p-12 text-center">
                ...
                <p class="text-sm font-bold text-gray-400">
                    Une question ? Écrivez-nous à <a href="mailto:contact@savoirsvivants.fr" class="text-teal-600 hover:text-slate-900 transition-colors">contact@savoirsvivants.fr</a>
                </p>
            </div>
            @endif
    ...
    return {
        ...
        emptyTuteur() {
            return {
                ...
                adhere: false, rentre_fin: false, rentre_annul: false,
