...
<head>
    ...
    <style>
        ...
        .section-title {
            color: #16987C;
            border-bottom: 2px solid #16987C;
            padding-bottom: 5px;
            font-size: 15px;
            font-weight: bold;
            text-transform: uppercase;
        ...
        /* Style pour les tableaux listes (paiements, présences) */
        .table-striped th {
...
<body>
    ...
    <table>
        <tr>
            ...
            <td>{{ $adherent->date_naiss ? $adherent->date_naiss->format('d/m/Y') : 'N/A' }}
                ({{ $adherent->age_courant ?? 'N/A' }} ans)</td>
        </tr>
        <tr>
            <th>Genre</th>
        ...
        <tr>
            ...
            <td>{{ $adherent->occupation ?? 'N/A' }}
                {{ $adherent->etablissement ? '(' . $adherent->etablissement . ')' : '' }}</td>
        </tr>
    </table>
        ...
        <table>
            @if ($adherent->problemes_sante)
                <tr>
                    ...
                    <td style="color: #e11d48;">{{ $adherent->problemes_sante }}</td>
                </tr>
            @endif
            @if ($adherent->allergies)
                <tr>
                    <th>Allergies</th>
                    <td style="color: #e11d48;">{{ $adherent->allergies }}</td>
                </tr>
            @endif
            @if ($adherent->restrictions_alimentaires)
                <tr>
                    <th>Restrictions alimentaires</th>
                    <td>{{ $adherent->restrictions_alimentaires }}</td>
                </tr>
            @endif
        </table>
    @endif
    ...
    @if ($adherent->tousLesTuteurs && $adherent->tousLesTuteurs->count() > 0)
        ...
        @foreach ($adherent->tousLesTuteurs as $tuteur)
            <table style="background-color: #f8fafc; border-radius: 8px; margin-bottom: 15px;">
                    ...
                    <td colspan="2" style="border: none; padding-bottom: 0;">
                        ...
                        <span style="float: right; font-size: 10px; text-transform: uppercase;">
                            @if ($tuteur->type === 'parent_tuteur')
                                REPRÉSENTANT LÉGAL
                            @elseif($tuteur->type === 'autre_autorise')
                                PERSONNE AUTORISÉE
                            @else
                                NON AUTORISÉ(E)
                            @endif
                ...
                <tr>
                    <td style="border: none; color: #666; font-weight: normal; padding-top: 5px;">Tél : <strong
                            style="color: #333;">{{ $tuteur->tel ?? 'N/A' }}</strong></td>
                    ...
                    <tr>
                        <td colspan="2" style="border: none; padding-top: 5px;">
                            <div class="signature-box" style="margin: 5px 0 5px auto;">
                                <img src="{{ $tuteur->signature }}" alt="Signature du représentant légal"
                                    class="signature-img">
                                <p class="signature-text">
                                    Signé le
                                    {{ $tuteur->date_signature ? \Carbon\Carbon::parse($tuteur->date_signature)->format('d/m/Y') : 'N/A' }}
                                </p>
                            </div>
                        </td>
                    </tr>
    ...
    <table>
        <tr>
            <th>Autorisations</th>
            <td>
                <span class="tag {{ $adherent->communication ? 'tag-green' : 'tag-red' }}">Image :
                    {{ $adherent->communication ? 'OUI' : 'NON' }}</span>
                <span class="tag {{ $adherent->bulletin ? 'tag-green' : 'tag-red' }}">Newsletter :
                    {{ $adherent->bulletin ? 'OUI' : 'NON' }}</span>
                <span class="tag {{ $adherent->manif ? 'tag-green' : 'tag-red' }}">Manifestations :
                    {{ $adherent->manif ? 'OUI' : 'NON' }}</span>
            </td>
        </tr>
        ...
        <table class="table-striped">
            ...
            <tbody>
                @foreach ($adherent->paiements->sortByDesc('date_paiement') as $paiement)
                    <tr>
                        <td>{{ $paiement->date_paiement ? \Carbon\Carbon::parse($paiement->date_paiement)->format('d/m/Y') : 'N/A' }}
                        </td>
                        <td><strong>{{ $paiement->source }}</strong></td>
                        <td style="color: #16987C; font-weight: bold;">
                            {{ number_format($paiement->montant, 2, ',', ' ') }} &euro;</td>
                        <td style="font-size: 11px;">{{ $paiement->commentaire ?? '-' }}</td>
                    </tr>
                @endforeach
                <tr>
                    <td colspan="2" style="text-align: right; border: none; padding-top: 15px;"><strong>TOTAL
                            ENCAISSÉ :</strong></td>
                    <td colspan="2"
                        style="border: none; padding-top: 15px; color: #16987C; font-size: 16px; font-weight: bold;">
    ...
    @if (isset($presences) && $presences->count() > 0)
        ...
        <table class="table-striped">
            ...
            <tbody>
                @foreach ($presences as $seance)
                    @php
                        $statut = strtolower($seance->statut_presence ?? 'présent');
                        $estAbsent = $statut === 'absent';
                        $tagClass = $estAbsent ? 'tag-red' : 'tag-green';
                        $label = $estAbsent ? 'Absent' : 'Présent';
                        $dateSeance = $seance->date ? \Carbon\Carbon::parse($seance->date)->format('d/m/Y') : 'N/A';
                    @endphp
                    <tr>
                        <td>{{ $dateSeance }}</td>
                        <td><strong>{{ $seance->activite->nom ?? 'Activité inconnue' }}</strong></td>
                        <td>
