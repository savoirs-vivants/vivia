    ...
    <div x-show="open"
        ...
        <div class="bg-white rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] w-full max-w-md pointer-events-auto overflow-hidden"
            ...
            <div class="p-5 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <template x-if="adherent.isPartiel">
                    <div class="space-y-4">
                        ...
                        <div>
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Solde en cours</p>
                        ...
                        <div class="space-y-3">
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Enregistrer un versement</p>
                            <div class="flex gap-2">
                                <div class="flex-1">
                                    <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Montant reçu</label>
                                    <div class="relative">
                                        <input type="number"
                                               x-model="montantVersement"
                                               @input="calculerResteApresVersement()"
                                               step="0.01" min="0.01"
                                               placeholder="0,00"
                            ...
                            <div>
                                <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Moyen de paiement</label>
                ...
                {{-- ===== MODE VALIDATION (adhérent En attente) ===== --}}
                <template x-if="!adherent.isPartiel">
                    <div class="space-y-4">
            ...
            <div class="flex items-center justify-between px-5 py-4 border-t border-gray-100 bg-gray-50/50">
                ...
                {{-- Formulaire versement (mode Partiel) --}}
                <form x-show="adherent.isPartiel" :action="adherent.versementUrl" method="POST">
                    ...
                    <button type="submit"
                            :disabled="!montantVersement || parseFloat(montantVersement) <= 0"
                ...
                {{-- Formulaire validation (mode En attente) --}}
                                    ...
                                    <td class="px-4 py-4 text-right">
                                        @if ($tab === 'partiel')
                                            @php
                                                ...
                                                $pctPartiel    = $totalPartiel > 0 ? min(100, round(($versePartiel / $totalPartiel) * 100)) : 0;
                                            @endphp
                                            <div class="flex flex-col items-end gap-1">
                                                <div>
                                                    <span class="font-black text-sm text-emerald-600">{{ number_format($versePartiel, 2, ',', ' ') }} €</span>
                                                    <span class="text-xs text-gray-400"> / {{ number_format($totalPartiel, 2, ',', ' ') }} €</span>
                                                </div>
                                                <div class="w-20 bg-gray-100 rounded-full h-1">
                                                    <div class="bg-[#16987C] h-1 rounded-full" style="width: {{ $pctPartiel }}%"></div>
                                                </div>
                                                <span class="text-[10px] text-amber-500 font-bold">reste {{ number_format($restePartiel, 2, ',', ' ') }} €</span>
                                            </div>
                                        @else
                                            @php
...
function adherentOverlay() {
    return {
        ...
        calculerResteApresVersement() {
            ...
            const newReste   = Math.max(0, resteBrut - versement);
            this.resteApresVersement = newReste.toFixed(2).replace('.', ',') + ' €';
        ...
        adherent: {
            isStructure: false, isPartiel: false, id: null, nom: '', initiales: '', couleur: '', meta: '',
            source: '', sourceClass: '', montant: '', refFacture: '',
            datePaiement: '', activites: [], commentaire: '',
            versementUrl: '', dejaVerse: '', resteDu: '', resteDuBrut: 0, dejaVerseBrut: 0, montantBrut: 0,
        },
    }
}
</script>
